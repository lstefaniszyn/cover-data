...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "data trapped in scanned images"
    - topic: "insight"
      decision: "real scans are crooked, not clean grids — naive row detection breaks on distorted table geometry, undermining both person-finding and accurate redaction of everyone else"
    - topic: "persona scope"
      decision: "a specific role inside one org — single-org internal tool, not multi-tenant"
    - topic: "cost today"
      decision: "manual redaction in an image/PDF editor — slow and error-prone, worsened by non-straight table rows/columns"
    - topic: "auth model"
      decision: "single user, single device for MVP — no login/auth layer"
    - topic: "role separation"
      decision: "flat — moot for MVP since there's only one user; deferred to Non-Goals"
  frs_drafted: 9
...
### Secondary
- A basic audit log records who searched for/generated what, and when. (Note: the preview criterion originally here was promoted to a must-have FR — see FR-007 and its Socrates note.)

### Guardrails
- Redacted data must not be recoverable via text extraction or similar trivial techniques — true PDF redaction, not a visual cover-up.
- The original source document is never modified; the anonymized PDF is always a separate output artifact.
...
- FR-001: User points the system at an image file already on disk (one scanned document page). No upload flow or PDF-to-image conversion for v1. Priority: must-have
  > Socrates: Counter-argument considered: real debtor lists arrive as scanned PDFs, so skipping PDF entirely just defers that work. Resolution: kept as the deliberate MVP shortcut; PDF ingestion (render pages → images) is captured as an explicit fast-follow, not dropped (see Forward: technical-roadmap).
...
- Every other row's data is unrecoverable from the output PDF by any trivial means (copy-paste, text extraction, zoom/contrast tricks on a visual-only cover)
...
- PDF ingestion (rendering PDF pages to images, upload flow) is explicitly deferred past the MVP, not dropped — FR-001 is a deliberate shortcut assuming an image file is already on disk. A fast-follow should add real PDF-in → image-out ingestion once the core search/redact loop is proven.
